...
$(document).ready(function () {
	...
	$(window).scroll(function (event) {
		...
		for (var key in sections) {
			...
			for (var prop in obj) {
				...
				if (prop == 'sectionTarget') {
					targetSection = obj['target'];
					offset = obj[prop];

					if (scrolled > offset) {
						sectionIsActive = true;
						targetSection = obj['target'];
						biggerThan.push(offset);
					}
				}
			}
		}
		...
		if (sectionIsActive) {
			currentSection = Math.max(...biggerThan);
	...
	$(document).on('click', '.link', function () {
		...
		var navWrapper = $(this).closest('.navigation-wrapper');
		var hamburger = $(navWrapper).siblings('.hamburger-container');

		if (navWrapper.hasClass('active')) {

			$(hamburger).addClass('animate-reverse');
	...
	$('.code-wrapper').each(function () {
		var btn = '<div class="copy-code"><span>copy code</span></div>';
		$(this).append(btn);
	});

	$(document).on('click', '.copy-code', function () {

		var test = $(this).siblings('pre').find('code'),
			$temp = $("<input>"), // create temp input
			$div = $('<div>'); // create temp input
		
		$("body").append($div);
		
		$div.html(test.html()); // value of input = code
		$div.find('.comment').remove(); 
		test = $div.text();

		$("body").append($temp);
		$temp.val(test).select(); // value of input = code
		document.execCommand("copy"); // copy the input
		$temp.remove(); // delete the input
		$div.remove(); // delete the input
	...
	// svg background height fix 
	// var $usp = $('.usp');
	// var height = $usp.height();
	// var wrapperHeight = $usp.children('.wrapper').height();

	// var heightAdd = wrapperHeight - height;

	// if (heightAdd > 0) {
	// 	$usp.css('height', wrapperHeight * 1.25 + 'px');
	// }
	...
	$('th').click(function(){
		...
		if (!this.asc) {
			rows = rows.reverse()
			$(this).find('.arrow-toggle').css('transform', 'rotate(180deg)');
